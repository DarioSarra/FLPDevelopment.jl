...
cas_df = filter(r->
    r.Gen == "Rbp4-cre" &&
    r.ProtocolSession == 1  &&
    r.AfterLast <= quantile(Cas_s.AfterLast,0.95),Cas_s)
gd1 = groupby(cas_df,[:Gen,:Virus,:MouseID])
c1 = combine(:AfterLast => mean => :AfterLast,gd1)
c1[!,:xpos] = [v == "tdTomato" ? 1 : 2  for v in c1.Virus]
c2 = combine(groupby(c1,[:Gen, :Virus])) do df
    (Mean = mean(df.AfterLast), SEM = sem(df.AfterLast))
end
@df c2 scatter(1:nrow(c2),:Mean, yerror = :SEM,
...
age_df = filter(r->
    r.ProtocolSession == 1 &&
    r.AfterLast <= quantile(Cas_s.AfterLast,0.95),
    Age_s)
